class BoardsController < ApplicationController
  ...
  def show
    ...
    all_posts = Post.where({ :board_id => @the_board.id }).order({ :expires_on => :asc })

    @active_posts = all_posts.where(["expires_on >= ?", today])
    @expired_posts = all_posts.where(["expires_on < ?", today])
